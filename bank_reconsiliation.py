...
#     print("📄 Saving selected pages from PDF with original quality...")

#     # Load PDF and validate page numbers
#     reader = PdfReader(input_pdf_path)
#     total_pages = len(reader.pages)
#     valid_page_numbers = [p for p in page_numbers if 1 <= p <= total_pages]

#     if not valid_page_numbers:
#         print("❌ No valid pages to save. Aborting.")
#         return None

#     os.makedirs(output_dir, exist_ok=True)
#     input_filename = os.path.splitext(os.path.basename(input_pdf_path))[0]
#     enhanced_pdf_path = os.path.join(output_dir, f"{input_filename}_selected_pages.pdf")

#     try:
#         with tempfile.TemporaryDirectory() as temp_dir:
#             # Extract selected pages into a temp PDF
#             temp_pdf_path = os.path.join(temp_dir, "temp_selected.pdf")
#             writer = PdfWriter()
#             for p in valid_page_numbers:
#                 writer.add_page(reader.pages[p - 1])
...
#             # Convert selected pages to high-resolution images (no enhancement)
#             images = convert_from_path(temp_pdf_path, dpi=450, fmt="png")

#             cleaned_images = []
#             for img in images:
#                 if img.mode in ["RGBA", "P"]:  # Convert only if necessary
#                     img = img.convert("RGB")
...
#         print(f"✅ PDF saved with original image quality: {enhanced_pdf_path}")
#         return enhanced_pdf_path
